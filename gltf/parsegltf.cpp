...
static void parseMeshesGltf(cgltf_data* data, std::vector<Mesh>& meshes, std::vector<std::pair<size_t, size_t> >& mesh_remap)
{
    ...
    printf("Initial meshes size: %zu, capacity %zu", meshes.size(), meshes.capacity());
    ...
    for (size_t mi = 0; mi < data->meshes_count; ++mi)
    {
        ...
        for (size_t pi = 0; pi < mesh.primitives_count; ++pi)
        {
            ...
            printf("Meshes size before push_back: %zu \n", meshes.size());
            printf("Meshes capacity before push_back: %zu \n", meshes.capacity());
            ...
            Mesh m = {
                    ...
                    nullptr,                          // material
                    nullptr,                          // skin
                    {},                               // type (initialize with appropriate value)
                    {},                               // streams
                    {},                               // indices
                    0,                                // targets
                    {},                               // target_weights
