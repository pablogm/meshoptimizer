...
void encodeImages(std::string* encoded, const cgltf_data* data, const std::vector<ImageInfo>& images, const char* input_path, const Settings& settings)
{
    ...
    std::string temp_prefix = getTempPrefix(input_path);
...
UIImage* imageFromCgltfImage (cgltf_image& image) {
    if (image.uri) {
        NSString *imagePath = [NSString stringWithUTF8String:image.uri];
        NSData *imageData = [NSData dataWithContentsOfFile:imagePath];
        return [UIImage imageWithData:imageData];
    } else if (image.buffer_view) {
        cgltf_buffer_view *bufferView = image.buffer_view;
        const char *dataPtr = (const char *)bufferView->buffer->data;
        NSData *imageData = [NSData dataWithBytes:dataPtr + bufferView->offset length:bufferView->size];
        return [UIImage imageWithData:imageData];
    }
    return nil;
}

void saveImageAsPNG(UIImage* image, NSString* fileName) {
    NSData *pngData = UIImagePNGRepresentation(image);
    if (pngData) {
        NSString *path = [NSHomeDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"Documents/%@.png", fileName]];
        [pngData writeToFile:path atomically:YES];
        NSLog(@"Saved image to %@", path);
    } else {
        NSLog(@"Failed to create PNG representation of the image.");
    }
}

void saveImage(cgltf_image& image, int job_id) {
    UIImage *img = imageFromCgltfImage(image);
    if (img) {
        NSString *fileName = [NSString stringWithFormat:@"myImage%d", job_id];
        saveImageAsPNG(img, fileName);
    }
}
