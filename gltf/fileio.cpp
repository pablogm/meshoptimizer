...
{
    // Make a copy of fullPath because dirname() may modify it
    char *fullPathCopy = strdup(fullPath);
    // Use dirname() to remove the last component (file name)
    std::string dirPath = dirname(fullPathCopy);
    free(fullPathCopy);

    std::string tempPrefix = dirPath + getTempPrefix();
